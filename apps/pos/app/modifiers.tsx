        ...
        options: [
          {
            ...
            name: option,
            priceDelta: { amount: price || '0', currency: 'LAK' as const },
          },
        ],
      };
      if (editingId) await updateModifierGroup(editingId, input);
      else await createModifierGroup(input);
      setName('');
      setOption('');
      setPrice('');
      setEditingId(null);
  ...
  return (
    <View style={styles.page}>
      ...
      <View style={styles.card}>
        <TextInput
          placeholder="ຊື່ກຸ່ມ ເຊັ່ນ ຄວາມຫວານ"
          value={name}
        ...
        <Text style={styles.button} onPress={() => void save()}>
          {editingId ? 'ອັບເດດ Modifier' : '+ ສ້າງ Modifier'}
        </Text>
      ...
      {loading ? (
        ...
        groups.map((group) => (
          <View style={styles.row} key={group.id}>
            ...
            <Text style={styles.meta}>{group.options.map((item) => item.name).join(' · ')}</Text>
            <Text
              style={styles.link}
              onPress={() => {
            ...
            <Text
              ...
              onPress={() =>
                void deleteModifierGroup(group.id)
                  .then(reload)
                  .catch(() => setMessage('ລຶບບໍ່ໄດ້: Modifier ນີ້ມີປະຫວັດການຂາຍ'))
              }
            >
              ລຶບ
            </Text>
...
const styles = StyleSheet.create({
  ...
  message: { color: lightColors.primary },
  row: { backgroundColor: lightColors.surface, padding: 14, borderRadius: 12, marginBottom: 8 },
  name: { color: lightColors.text, fontFamily: 'NotoSansLao_700Bold' },
