...
import { createCatalogCategory, createCatalogItem, deleteCatalogCategory, deleteCatalogItem, listCatalogCategories, listCatalogItems, updateCatalogItem } from '../src/auth/auth-client';
export default function ItemsScreen() {
  ...
  const createItem = async () => {
    setMessage('');
    try {
      const unit = { name: unitName || 'ອັນ', multiplierToBase: multiplier || '1', price: { amount: price, currency: 'LAK' as const }, sku: sku || undefined, barcode: barcode || undefined };
      if (editingId) await updateCatalogItem(editingId, { name, units: [{ ...unit, id: items.find((item) => item.id === editingId)?.units[0]?.id }] });
      else await createCatalogItem({ name, baseUnitName: unit.name, trackStock: false, units: [unit] });
      setItems(await listCatalogItems()); setName(''); setPrice(''); setBarcode(''); setMessage('ສ້າງສິນຄ້າສຳເລັດ');
      setUnitName('ອັນ'); setMultiplier('1'); setSku(''); setEditingId(null);
    } catch { setMessage('ສ້າງສິນຄ້າບໍ່ສຳເລັດ'); }
  };
  const editItem = (item: (typeof items)[number]) => { const unit = item.units[0]; setEditingId(item.id); setShowCreate(true); setName(item.name); setPrice(String(unit?.priceAmount ?? '')); setUnitName(unit?.name ?? 'ອັນ'); setMultiplier(String(unit?.multiplierToBase ?? '1')); setSku(unit?.sku ?? ''); setBarcode(unit?.barcode ?? ''); };
  const removeItem = async (id: string) => { try { await deleteCatalogItem(id); setItems(await listCatalogItems()); setMessage('ປິດໃຊ້ງານສິນຄ້າແລ້ວ'); } catch { setMessage('ລຶບສິນຄ້າບໍ່ສຳເລັດ'); } };
  const removeCategory = async (id: string) => { try { await deleteCatalogCategory(id); setCategories(await listCatalogCategories()); setMessage('ລຶບໝວດໝູ່ແລ້ວ'); } catch { setMessage('ລຶບບໍ່ໄດ້: ອາດຍັງມີສິນຄ້າໃຊ້ຢູ່'); } };
  ...
  return (
    <View style={styles.safe}>
      ...
      <Text onPress={() => { setEditingId(null); setShowCreate((value) => !value); }} style={styles.action}>{showCreate ? '− ປິດຟອມ' : '+ ເພີ່ມສິນຄ້າ'}</Text>
      {showCreate ? <View style={styles.form}>
        <TextInput placeholder="ຊື່ສິນຄ້າ" value={name} onChangeText={setName} style={styles.input} />
        <TextInput placeholder="ຊື່ໜ່ວຍ (ເຊັ່ນ ອັນ/ແພັກ)" value={unitName} onChangeText={setUnitName} style={styles.input} />
        <TextInput placeholder="Conversion ໄປຫາໜ່ວຍຖານ" value={multiplier} onChangeText={setMultiplier} keyboardType="decimal-pad" style={styles.input} />
        <TextInput placeholder="ລາຄາ (LAK)" value={price} onChangeText={setPrice} keyboardType="number-pad" style={styles.input} />
        <TextInput placeholder="SKU (ຖ້າມີ)" value={sku} onChangeText={setSku} style={styles.input} />
        <TextInput placeholder="Barcode (ຖ້າມີ)" value={barcode} onChangeText={setBarcode} style={styles.input} />
        <Text onPress={() => void createItem()} style={styles.button}>{editingId ? 'ອັບເດດສິນຄ້າ' : 'ບັນທຶກສິນຄ້າ'}</Text>
        ...
        {categories.length ? <View>{categories.map((category) => <View key={category.id} style={styles.categoryRow}><Text style={styles.meta}>{category.name}</Text><Text onPress={() => void removeCategory(category.id)} style={styles.delete}>ລຶບ</Text></View>)}</View> : null}
          ...
          renderItem={({ item }) => (
            <View style={styles.row}>
              <View>
                ...
                <View><Text style={styles.price}>{String(item.units[0]?.priceAmount ?? 0)} ₭</Text><Text onPress={() => editItem(item)} style={styles.edit}>ແກ້ໄຂ</Text><Text onPress={() => void removeItem(item.id)} style={styles.delete}>ປິດໃຊ້ງານ</Text></View>
            </View>
          )}
        />
      )}{' '}
