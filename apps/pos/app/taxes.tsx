...
import { createTax, deleteTax, listTaxes, updateTax } from '../src/auth/auth-client';
import { lightColors } from '../src/design/tokens';

export default function TaxesScreen() {
  const [taxes, setTaxes] = useState<Awaited<ReturnType<typeof listTaxes>>>([]);
  ...
  return (
    <View style={styles.page}>
      <Text style={styles.title}>ພາສີ</Text>
      <View style={styles.card}>
        <TextInput value={name} onChangeText={setName} placeholder="ຊື່ພາສີ" style={styles.input} />
        <TextInput
          value={rate}
      ...
      {taxes.map((tax) => (
        <View style={styles.row} key={tax.id}>
          <Text style={styles.name}>{tax.name}</Text>
          <Text style={styles.meta}>
            {(tax.rateBasisPoints / 100).toFixed(2)}% · {tax.mode}
          </Text>
          <Text
            style={styles.link}
            onPress={() => {
          ...
          <Text style={styles.delete} onPress={() => void deleteTax(tax.id).then(reload)}>
            ປິດໃຊ້ງານ
          </Text>
        </View>
      ))}
