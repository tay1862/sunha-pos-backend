...
export async function listSyncOperations(status?: 'PENDING' | 'ACKED' | 'FAILED_REVIEW') { const token = await getAccessToken(); return apiRequest<Array<{ operationId: string; type: string; status: string; attemptCount: number; lastError?: string | null; createdAt: string }>>(`/sync/operations${status ? `?status=${status}` : ''}`, { accessToken: token ?? undefined }); }
export async function retrySyncOperation(operationId: string) { const token = await getAccessToken(); return apiRequest(`/sync/operations/${operationId}/retry`, { method: 'POST', accessToken: token ?? undefined }); }
export async function listDevices() { const token = await getAccessToken(); return apiRequest<Array<{ id: string; status: string; offlineLeaseExpiresAt?: string | null }>>('/devices', { accessToken: token ?? undefined }); }
export async function getDeviceLease(deviceId: string) { const token = await getAccessToken(); return apiRequest<{ canSellOffline: boolean; offlineLeaseExpiresAt?: string | null }>(`/devices/${deviceId}/lease`, { accessToken: token ?? undefined }); }
export async function getSellingPolicy() { const token = await getAccessToken(); return apiRequest<{ canChargeOffline: boolean; sellingDeviceCount: number; offlineLeaseExpiresAt?: string | null }>('/devices/current/policy', { accessToken: token ?? undefined }); }
