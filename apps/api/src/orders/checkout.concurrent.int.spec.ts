...
describe.skipIf(!runIntegration)('PostgreSQL concurrent checkout', () => {
  ...
  it('reconciles checkout, immutable receipt and full refund ledgers', async () => {
    ...
    const refund = await refunds.refund(tenantId, employeeId, deviceId, { orderId: order.id, managerEmployeeId: managerId, managerPin: '123456', reason: 'integration test' });
    ...
    await expect(refunds.refund(tenantId, employeeId, deviceId, { orderId: order.id, managerEmployeeId: managerId, managerPin: '123456', reason: 'duplicate' })).rejects.toThrow('ORDER_NOT_REFUNDABLE');
  });
  ...
  it('opens one shared shift, records cash movement and closes with blind expected cash', async () => {
    ...
    await shifts.movement(tenantId, employeeId, deviceId, { amount: '100', type: 'CASH_IN', reason: 'float' });
    await expect(shifts.open(tenantId, employeeId, deviceId, { openingAmount: '1' })).rejects.toThrow('SHIFT_ALREADY_OPEN');
